...
public class UserTests {
    ...
    public void getTextForStartRestMessageReturnsCorrectTextWhenRestBetweenApproaches(){
        User user = new User();
        user.setLevel(WorkoutMaker.getBeginLevel());
        user.setTargetGroups(new String[]{WorkoutMaker.getLegsGroup()});
   ...
   /* @Test
    public void getTextForStartRestMessageReturnsCorrectTextWhenWorkoutIsFinished(){
        User user = new User();
        user.setLevel("новичок");
        user.setTargetGroups(new String[]{"ноги"});
        user.setWorkout(new WorkoutMaker(user.getLevel(), user.getTargetGroups()).createWorkout());
        user.setCurrentApproach(8);
